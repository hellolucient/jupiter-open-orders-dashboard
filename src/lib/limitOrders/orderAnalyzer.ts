...
// Helper functions for buffer formatting
...
function displayField(name: string, slice: Buffer, outputStream: WriteStream) {
...
async function displayU64Field(
  ...
  const value = BigInt(left) + (BigInt(right) << BigInt(32));
...
function displayOptionI64Field(name: string, slice: Buffer, outputStream: WriteStream) {
  const format = formatSlice(slice);
...
function displayU16Field(name: string, slice: Buffer, outputStream: WriteStream) {
  const format = formatSlice(slice);
...
function displayI64Field(name: string, slice: Buffer, outputStream: WriteStream) {
  const format = formatSlice(slice);
...
function displayU8Field(name: string, slice: Buffer, outputStream: WriteStream) {
  const format = formatSlice(slice);
...
export async function analyzeOrder(orderAccount: string, rawData: string, outputStream: WriteStream, connection: Connection) {
